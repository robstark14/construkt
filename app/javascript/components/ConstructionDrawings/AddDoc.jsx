...
function AddDoc() {
    ...
    const createDoc= async (e) =>{
        ...
        try{
            ...
            // navigate("/")
            console.log('posted!!');
            console.log(values.attachments.url);
            console.log(values.attachments);
            console.log(values.attachments.name);
            console.log(values.attachments['url']);
            console.log(values.attachments['path']);
            console.log(values.attachments.path);
   ...
   const generateLevels = () =>{
       let i;
       for(i=1; i <= 30; i++){
       //  setLocationArr(...locationArr, `L${i}`)
       levelsArr.push(`L${i}`)

   }
   }
   const fetchCurrentUser= async () =>{
    const apiEndpointGetUserData = `/api/user`
      try{
        
          const response = await fetch(apiEndpointGetUserData)
          const data = await response.json()
          setUser({userName: data['user'].user_name, documentNumber: data['user'].enum})
          setValues({ ...values, publishedBy: data['user'].user_name })
        ...
        <div className="grid gap-6 mb-6 md:grid-cols-3 ">
            ...
            <div className='w-fit'>
                ...
                <input
                 ...
                 type="text" readOnly className="border border-gray-300 text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="CD" required/>
            </div>
            <div className='w-fit'>
            ...
            <div className='w-fit'>
                ...
                <input name='company_from' type="text" value="Project Owner" readOnly className="border border-gray-300 text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:focus:ring-blue-500 dark:focus:border-blue-500" required/>
            </div>
            <div>
